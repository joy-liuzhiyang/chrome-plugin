...
const getBossResumeHTML = async () => {
    ...
    const currentPageKey = document.querySelector(".side-menu .router-link-active").getAttribute('ka');
    if (['menu-geek-recommend'].includes(currentPageKey)) {//推荐牛人
        const iFrameEle = document.querySelector('[name=\"recommendFrame\"]') as any;
        if (iFrameEle) {
            const resumeModalEle = iFrameEle.contentDocument.querySelector('.resume-dialog');
            if (!resumeModalEle) {
                const resumeDetailEle = await getDom(`document.querySelector('[name=\"recommendFrame\"]').contentDocument.querySelector("#resume-page .resume-item-content")`) as any;
                if (!resumeDetailEle) {
                ...
                obj.errorInfo = "请先打开简历详情,在尝试解析";
            }
        } else {
            obj.errorInfo = "请先打开简历详情,在尝试解析";
        }
    } else if (['menu-geek-search'].includes(currentPageKey)) {//搜索牛人
        const resumeModalEle = document.querySelector('.dialog-resume-full.search-resume');
        if (!resumeModalEle) {
            obj.errorInfo = "请先打开简历详情,在尝试解析";
        } else {
            const resumeDetailEle = await getDom(`document.querySelector('.dialog-resume-full.search-resume').querySelector(".resume-dialog")`) as any;
            if (!resumeDetailEle) {
                obj.errorInfo = "未检测到简历详情";
            } else {
                obj.fileContent = resumeDetailEle.outerHTML;
    ...
    } else if (['menu-im'].includes(currentPageKey)) {//沟通
        try {
            (document.querySelector('.conversation-hd-box').children[1] as any).click();
            const resumeDetailEle = await getDom(`document.querySelector('.resume-detail')`) as any;
            if (!resumeDetailEle) {
                obj.errorInfo = "未检测到简历详情";
            } else {
                obj.fileContent = resumeDetailEle.outerHTML;
...
export const webJudge = async (data: any, sendResponse: any) => {
    ...
    try {
        if (url.includes("zhaopin.com/resume/detail")) {//智联
            if (document.querySelector('.resume-detail--default')) {
                fileContent = document.querySelector('.resume-detail--default').outerHTML;
            }
        } else if (url.includes("51job.com/Candidate/ResumeViewFolderV2.aspx")) {//前程无忧
            if (document.querySelector('#divResume')) {
                fileContent = document.querySelector('#divResume').outerHTML;
            }
        } else if (url.includes("lagou.com/can/new/index.htm")) {//拉钩
            const modalEle = document.querySelector(".lg-modal-content");
            if (!modalEle) {
                errorInfo = '请先打开简历详情,在尝试解析';
            } else {
                fileContent = document.querySelector(".lg-modal-content").outerHTML;
            }
        } else if (url.includes("lagou.com/can/new/details.htm")) {//拉钩
            try {
                (document.querySelector('.rc-tabs-nav-animated').children[0].children[0] as any).click();
                //200ms 等待在线简历渲染
            ...
            url.includes('zhipin.com/web/boss/index')) {//boss 
...
/** background popup 专用 */
export const fetchHttpRequest = (url: string, requestType: 'get' | 'post', params: any, extraHeader: any = {}) => {
    return new Promise((resolve: any, reject: any) => {
        const reg = /^http(s)?:\/\/(.*?)\//;
        try {
            chrome.cookies.getAll({ url: "https://" + reg.exec(url)[2] }, cookies => {
                resolve(new http()[requestType](url, params, {
                    "X-XSRF-TOKEN": cookies.find((item: any) => item.name === 'XSRF-TOKEN').value,
                    ...extraHeader,
                }))
            });
        } catch (error) {
...
export function isRecruitPublic() {
    ...
    if (location.href.includes('.lagou.com/position/multiChannel/createPosition.htm')) {
        return 'lagou';
    }
    ...
    if (location.href.includes('.liepin.com/ejobedit/editsocietyejob?ejobActionType=publish')) {
        return 'liepin';
    }

    if (location.href.includes('.ganji.com/pub/pub.php') && location.href.includes('_pdt=zhaopin')) {
        return 'ganji';
    }
