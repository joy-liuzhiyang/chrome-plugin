...
class IrsFetchHttp {
    ...
    async fetchData(url: string, reqtype: any, options: any, extHeader = {}, file: any = false, payload: string = 'json') { //默认'json'，文本传值‘text’
        this.initHeader = {
        ...
        if (payload === 'json') {
            ...
            this.headers = !file ? this.headers = Object.assign(this.initHeader, {
                'Content-Type': (!file ? 'application/json' : 'multipart/form-data'),
    ...
    get = (url: string, options: any, extHeader: any = {}) => {
        ...
        return this.fetchData(fetchUrl, 'get', {}, extHeader).then(async (d: any) => {
            return {
                status: d.status,
                res: await d.json()
            }
    ...
    post = (url: string, options: any, extHeader: any = {}, file: any = false, payload: string = 'json') => {
        return this.fetchData(url, 'post', options, extHeader, file, payload).then(async (d: any) => {
            return {
                status: d.status,
                res: await d.json()
            }
    ...
    put = (url: string, options: any, extHeader: any = {}, file: any = false, payload: string = 'json') => {
        return this.fetchData(url, 'put', options, extHeader, file, payload).then(async (d: any) => {
            return {
                status: d.status,
                res: await d.json()
            }
    ...
    delete = (url: string, options: any, extHeader: any = {}) => {
        return this.fetchData(url, 'delete', options, extHeader).then(async (d: any) => {
            return {
                status: d.status,
                res: await d.json()
            }
