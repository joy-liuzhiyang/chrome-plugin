...
class ZhilianTransformClass extends transformBaseClass {
  ...
  getSubmitBtn = () => {
    let selector =
      ".km-button.km-control.km-ripple-off.km-button--primary.km-button--filled";
  ...
  setValue = () => {
    ...
    try {
      const formEle = document.querySelector(
        ".publish-form__inner.km-form"
      ) as any;

      if (formEle && formEle.__vue__) {
        if (this.result.positionName) {
          formEle.__vue__.model.jobTitle = this.result.positionName;
        }

        if (this.result.workExperience) {
          formEle.__vue__.model.workAge = this.result.workExperience;
        }

        if (this.result.education) {
          formEle.__vue__.model.education = this.result.education;
        }
        ...
        if (
          ...
          formEle.__vue__.model.minSalary = salaryMin;
          ...
          if (this.result.salaryMax) {
            ...
            if (salaryMax) {
              formEle.__vue__.model.maxSalary = salaryMax;
            }
          }
        }

        if (this.result.positionTotal) {
          formEle.__vue__.model.quantity = `${this.result.positionTotal}`;
        }

        if (this.result.salaryMonth) {
          formEle.__vue__.model.salaryMonths = this.result.salaryMonth;
        }
      }
      ...
      if (this.result.positionDescription) {
        ...
        editor && editor.focus();
        const vm = (
          document.querySelector(".publish-form__inner.km-form") as any
        ).__vue__;
        if (vm) {
          vm.$parent.$refs.jobEditor.setData(
            "<div>" +
    ...
    window.postMessage(
      {
        ...
        data: {
          ...
          name: formEle.__vue__.model.jobTitle,
        },
      },
      "/"
    );
